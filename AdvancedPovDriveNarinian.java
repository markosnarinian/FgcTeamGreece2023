...
@TeleOp(name = "AdvancedPovDriveNarinian")
public class AdvancedPovDriveNarinian extends LinearOpMode {
    /**
    ...
    public void runOpMode() {
        ...
        final int cfgLiftRetractedPosTolerance = 60;
        ...
        final int cfgHydrogenDoorOpenLiftFloor = 500;
        ...
        // ----MOTOR DIRECTIONS, RUN-MODES - BEGINNING----
...
//        rightMotor.setMode(DcMotor.RunMode.RUN_USING_ENCODER);
        ...
        // ----MOTOR DIRECTIONS, RUN-MODES - END----
...
//                         * cstHdhexTpsNoLoad - rightMotor.getVelocity()) * driveMotorPwrIncrementCoefficient);
            ...
            while (opModeIsActive() && SAFE_MODE_ACTIVATED) {
                ...
                ctlIntakeToggle = gamepad1.right_bumper;
                ctlIntakeReverse = gamepad1.square;
                ...
                // Enable turbo mode while left stick (throttle) button is held down
                ctlJsThrottleAxis *= ctlTurboModeEnable ? cfgMotorPowerGainHigh : cfgMotorPowerGainNormal;
                ctlJsSteeringAxis *= ctlTurboModeEnable ? cfgSteeringGainHigh : cfgSteeringGainNormal;
                ...
                if (intakeToggleMgr.buttonPress(ctlIntakeToggle, runtime.milliseconds())) {
                    // Invert motor state
                    if (intakeMotorPowerToSet == 0) { // If intake motor is off
                        intakeMotorPowerToSet = cfgIntakeMotorPower; // Set power target to predefined value
                        intakeMotorStateHrf = "ON"; // Motor state in human readable format
                    } else { // Else
                        intakeMotorPowerToSet = 0; // Set power target to 0
                        intakeMotorStateHrf = "OFF"; // Motor state in human readable format
                    }
                }
                // If reverse button is pressed, reverse the intake
                intakeMotor.setPower(ctlIntakeReverse ? -cfgIntakeMotorPower : intakeMotorPowerToSet); // If intake reverse button is pressed, then set motor power to negative intakeMotorPower;
